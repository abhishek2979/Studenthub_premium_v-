...
function PlanCard({ plan, currentPlan, premiumExpiry, onSelect }) {
...
function PaymentModal({ plan, user, onClose, onSuccess }) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const initRes = await premiumAPI.initiatePayment({ plan: plan.id });
      const { orderId, amount, paymentId, razorpayKeyId } = initRes.data;
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
            ...
            <span style={{ fontWeight:600, color:COLORS.text }}>{plan.name}</span>
          </div>
          <div style={{ display:'flex', justifyContent:'space-between', marginBottom:8 }}>
          ...
          <div style={{ display:'flex', justifyContent:'space-between', borderTop:`1px solid ${COLORS.border}`, paddingTop:8 }}>
            ...
            <span style={{ fontWeight:800, fontSize:18, color:COLORS.text }}>₹{plan.price}</span>
          </div>
        </div>
        ...
        {error && (
          <div style={{
            ...
            color:COLORS.red, fontSize:13, display:'flex', gap:8, alignItems:'center',
          }}>
        ...
        <div style={{ display:'flex', gap:10 }}>
          ...
          <button onClick={handlePay} disabled={loading} style={{
            ...
            {loading
              ? <><Spinner size={16}/> Opening checkout…</>
              : <><CreditCard size={16}/> Pay ₹{plan.price}</>
            }
          </button>
        </div>
  ...
  return (
    <div style={{ maxWidth:860, margin:'0 auto', padding:'32px 16px' }}>
      ...
      <div style={{ marginTop:48, background:COLORS.card, borderRadius:16, padding:24, border:`1px solid ${COLORS.border}` }}>
        <h3 style={{ fontSize:16, fontWeight:700, color:COLORS.text, marginBottom:16 }}>FAQs</h3>
        {[
          ['Can I upgrade from Premium 1 to Premium 2?', 'Yes — pay for Premium 2 and your plan upgrades immediately.'],
          ['What video formats are supported?', 'MP4, MOV, AVI, MKV and WebM up to 500 MB each.'],
          ['How do live lectures work?', 'Schedule a class with a Google Meet or Zoom link. Students see it and can join directly.'],
          ['Is the payment secure?', 'Yes — payments are processed by Razorpay with 256-bit SSL encryption.'],
        ].map(([q, a]) => (
