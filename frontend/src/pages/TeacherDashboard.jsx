...
import {
  ...
  StickyNote, HelpCircle, Crown, Video, Radio
} from 'lucide-react';
...
import VideoLecturesTab  from './VideoLecturesTab';
...
const TABS = [
  ...
  { id:'videos',      icon:Video,         label:'Video Lectures', premiumMin:'premium1' },
  { id:'live',        icon:Radio,         label:'Live Classes',   premiumMin:'premium2' },
  { id:'premium',     icon:Crown,         label:'Premium Plans'   },
];
        ...
        <div style={{padding:"20px 20px 16px",borderBottom:"1px solid rgba(255,255,255,.08)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:9}}>
            ...
            <div style={{overflow:"hidden",flex:1}}>
              ...
              <div style={{color:"rgba(240,237,233,.45)",fontSize:10,letterSpacing:"0.3px"}}>Teacher · {user.googleId ? 'Google' : 'Email'}</div>
            </div>
          </div>
        </div>
          ...
          {TABS.map(t => (
            <button key={t.id} onClick={() => { setTab(t.id); if(isMobile) onClose(); }}
              style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"11px 12px",borderRadius:10,border:"none",cursor:"pointer",
                ...
                color:tab===t.id?"#F0EDE9":t.id==='premium'?"rgba(245,158,11,.7)":"rgba(240,237,233,.5)",
                fontWeight:tab===t.id?600:400,fontSize:14,
                marginBottom:2,textAlign:"left",transition:"all .15s"}}>
              <t.icon size={16} color={t.id==='premium'&&tab!==t.id?"rgba(245,158,11,.7)":undefined}/>
              {t.label}
              {t.premiumMin && tab!==t.id && (
                <span style={{
                  marginLeft:"auto",fontSize:9,fontWeight:700,padding:"2px 5px",
                  borderRadius:4,background:"rgba(245,158,11,.2)",color:"#F59E0B",
                }}>PRO</span>
              )}
...
function PremiumGate({ minPlan, onUpgrade }) {
  const isPrem2 = minPlan === 'premium2';
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:60,textAlign:'center'}}>
      ...
      <h2 style={{fontWeight:800,fontSize:22,color:'#1C1917',margin:'0 0 8px'}}>
        {isPrem2 ? 'Premium 2 Required' : 'Premium 1 Required'}
      </h2>
      <p style={{color:'#78716C',fontSize:15,maxWidth:380,lineHeight:1.6,margin:'0 0 24px'}}>
        {isPrem2
          ? 'Schedule and run live classes with your students. Upgrade to Premium 2 to unlock this feature.'
          : 'Upload video lectures organised by subject and day. Upgrade to Premium 1 to unlock this feature.'
        }
      </p>
      <button onClick={onUpgrade} style={{
        ...
        <Crown size={16}/> View Premium Plans
      </button>
    </div>
  );
}
    ...
    <div style={{display:"flex",minHeight:"100vh",background:C.bg}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",minWidth:0}}>
        ...
        <main style={{flex:1,padding:isMobile?16:32,overflow:"auto",maxHeight:isMobile?"none":"100vh",background:C.bg}}>
          ...
          {tab==='notes'       && <NotesTab       showNotif={showNotif}/>}
          {tab==='assignments' && <AssignmentsTab showNotif={showNotif}/>}
          {tab==='quiz'        && <QuizTab        showNotif={showNotif}/>}
          {tab==='premium'     && <PremiumPage />}
          {tab==='videos'      && (
            user?.premiumActive && ['premium1','premium2'].includes(user?.premiumPlan)
              ? <VideoLecturesTab teacher={user}/>
              : <PremiumGate minPlan="premium1" onUpgrade={() => setTab('premium')}/>
          )}
          {tab==='live'        && (
            user?.premiumActive && user?.premiumPlan === 'premium2'
              ? <LiveLectureTab />
              : <PremiumGate minPlan="premium2" onUpgrade={() => setTab('premium')}/>
          )}
        </main>
      </div>
    </div>
