...
const createStudent = asyncHandler(async (req, res) => {
  ...
  // Roll and email uniqueness scoped per teacher
  if (await Student.findOne({ roll, createdBy: req.user._id }))
    { res.status(400); throw new Error('Roll number already exists in your class'); }
  if (await Student.findOne({ email, createdBy: req.user._id }))
    { res.status(400); throw new Error('Email already used by one of your students'); }

  // Username must be globally unique (students log in by username)
  if (await User.findOne({ username: cleanUsername }))
    { res.status(400); throw new Error(`Username "${cleanUsername}" is already taken`); }

  const student = await Student.create({ name, roll, email, class: cls, phone, address, dob, guardian, createdBy: req.user._id });

  await User.create({
    name,
    email: `${cleanUsername}@studenthub.local`,  // internal email — avoids cross-teacher conflicts
...
const getStudentStats = asyncHandler(async (req, res) => {
  const teacherFilter = { createdBy: req.user._id };
  const [total, active, inactive] = await Promise.all([
    Student.countDocuments(teacherFilter),
    Student.countDocuments({ ...teacherFilter, status: 'active' }),
    Student.countDocuments({ ...teacherFilter, status: 'inactive' }),
  ]);
