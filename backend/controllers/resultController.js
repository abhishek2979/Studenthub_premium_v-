...
// Teachers can get any of their students; students can only get their own
const getStudentResults = asyncHandler(async (req, res) => {
  const { studentId } = req.params;
  const { examType, subject } = req.query;

  if (req.user.role === 'student') {
    // Students may only fetch their own results
    const myStudentRef = req.user.studentRef;
    if (!myStudentRef || myStudentRef.toString() !== studentId) {
      res.status(403); throw new Error('Access denied');
    }
  } else {
    // Teacher: confirm the student belongs to them
    const student = await Student.findOne({ _id: studentId, createdBy: req.user._id });
    if (!student) { res.status(404); throw new Error('Student not found'); }
  }
...
const getAllResults = asyncHandler(async (req, res) => {
  ...
  const studentQuery = { createdBy: req.user._id };
  if (cls) studentQuery.class = cls;
  const myStudents = await Student.find(studentQuery);
  const sIds = myStudents.map(s => s._id);

  const query = { student: { $in: sIds } };
  if (examType) query.examType = examType;
  if (subject)  query.subject  = subject;
...
// UPDATE result — fixed: 'fields' was undefined
const updateResult = asyncHandler(async (req, res) => {
  const result = await Result.findOne({ _id: req.params.id, createdBy: req.user._id });
  if (!result) { res.status(404); throw new Error('Result not found'); }

  const updatableFields = ['subject', 'marks', 'maxMarks', 'examType', 'examDate', 'remarks'];
  updatableFields.forEach(f => { if (req.body[f] !== undefined) result[f] = req.body[f]; });
...
const deleteResult = asyncHandler(async (req, res) => {
  ...
  if (result.documentPublicId) {
    await cloudinary.uploader.destroy(result.documentPublicId, { resource_type: 'auto' });
  }
